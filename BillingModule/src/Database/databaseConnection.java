...
"                    ORDER BY service_processed_date";
    ...
    public Vector<OCC> retrieveProcessedOCC(String msisdn, BillDateInterval interval) {

        connect();
        Vector<OCC> customers = new Vector();
        OCC occ;
        try {
            sqlcommand = "SELECT * FROM occ where msisdn= ? AND type_of_service = 'recurring_service' AND is_service_processed = true\n" +
"                    AND service_processed_date BETWEEN to_date(?,'YYYYMMDD') AND to_date(?,'YYYYMMDD')";
            preparedstatement = connection.prepareStatement(sqlcommand);
            preparedstatement.setString(1, msisdn);
            preparedstatement.setString(2, interval.getStartDate());
    ...
    public Boolean renewRecurringService(OCC occ){
        
        
        try {
            connect();
            sqlcommand = "SELECT renewRecurring(?, ?, ?, ?, ?)";
            preparedstatement.setString(1, occ.msisdn);
            preparedstatement.setInt(2,occ.one_rec_id);
            preparedstatement.setString(3, occ.type_of_service);
            preparedstatement.setBoolean(4, occ.is_service_processed);
            preparedstatement.setDate(5, occ.serviceprocessed_date);
            result = preparedstatement.executeQuery();
...
//            Vector<OCC> occs;
//            occs = db.getAllOcc(udr, new BillDateInterval("20200401", "20200430"));
//            for(OCC occ : occs){
